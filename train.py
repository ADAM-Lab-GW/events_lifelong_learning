...
https://github.com/GMvandeVen/brain-inspired-replay/blob/master/train.py
...
def train_cl(model, train_datasets, replay_mode="none", rnt=None, classes_per_task=None,
             ...
             generator=None, gen_iters=0, gen_loss_cbs=list(), feedback=False, reinit=False, args=None,
             only_last=False):
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        ...
        for batch_index in range(1, iters_to_use + 1):
            ...
            if Generative or Current:
                ...
                if previous_model.mask_dict is None:
        ...
        # Save habituation counters:
        # if isinstance(model, ContinualLearner) and args.habituation:
        #     habs_counter = model.habituation_container.habituation_counter.shape[0]
        #     habs.append(habs_counter)

    # Save habituation counters:
    if isinstance(model, ContinualLearner) and args.habituation:
        np.save("./store/results/habituation_counters_tasks-{}_dr-{}_topn-{}_si-{}_sic-{}".format(task_copy,
                                                                                 args.habituation_decay_rate,
                                                                          args.top_hab_neurons,
                                                                          args.si,
                                                                          args.si_c),
                model.habituation_container.habituation_counter.cpu().numpy())
