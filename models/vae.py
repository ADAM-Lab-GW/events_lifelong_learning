...
from models.utils.hidden_activation_container import HiddenActivationContainer
...
class AutoEncoder(ContinualLearner):
    ...
    def __init__(self, image_size, image_channels, classes,
                 ...
                 # -decoder
                 recon_loss='BCE', network_output="sigmoid", hidden=False,
                 dg_gates=False, dg_type="task", dg_prop=0., tasks=5, device='cuda',
                 # -classifer
                 classifier=True, classify_opt="beforeZ",
                 # -training-specific settings (can be changed after setting up model)
                 lamda_pl=0., lamda_rcl=1., lamda_vl=1., habituation_decay_rate=0, top_hab_neurons=0,  **kwargs):
        ...
        ##>----Decoder (= p[x|z])----<##
        out_nl = True if fc_layers > 1 else False   # TODO
        real_h_dim_down = h_dim if fc_layers > 1 else 1536  # TODO
        if self.dg_gates:
        ...
        fc_layer_sizes_down[0] = 1536  # TODO
        ...
        if self.dg_gates:
            self.fcD = MLP_gates(
                ...
                gate_size=self.gate_size, gating_prop=dg_prop, device=device,
                output=self.network_output if self.hidden else 'normal',
            )
        else:
            self.fcD = MLP(
                size_per_layer=[x for x in reversed(fc_layer_sizes_down)], drop=fc_drop, batch_norm=fc_bn, nl=fc_nl,
                gated=fc_gated, output=self.network_output if self.hidden else 'normal',
            )
        ...
        # Habituation
        self.habituation_container = HiddenActivationContainer(device, fc_units, habituation_decay_rate)
        self.top_hab_neurons = top_hab_neurons
        self.previous_task = 0
        self.current_he_activations = None
    ...
    def get_name(self):
        ...
        return "{}={}{}{}".format(self.label, z_label, class_label, decoder_label)
    ...
    def encode(self, x, current_task=None):
        ...
        # Habituation
        if self.habituation:
            self.habituation_container.update_habituation_counter(
                torch.topk(hE, math.floor(self.top_hab_neurons * self.fc_layer_sizes[-1]), dim=1).indices)
            hE.register_hook(lambda grad: grad * self.habituation_container.get_habituation_counter())

        # Slowness term
        if self.slowness:

            if current_task - self.previous_task > 1:
                self.previous_task = copy.deepcopy(current_task)
                self.habituation_container.reset_hidden_activations()
                self.habituation_container.reset_norm()

            if current_task != 1 and (current_task - self.previous_task > 1):

                last_task_activations = self.habituation_container.hidden_activations[-1]
                self.habituation_container.reset_hidden_activations()
                self.habituation_container.add_hidden_activations(last_task_activations)
                self.habituation_container.add_hidden_activations(self.current_he_activations)
                self.habituation_container.calculate_l1_slowness_term()
    ...
    def forward(self, x, gate_input=None, full=False, reparameterize=True, **kwargs):
        ...
        if full:
            ...
            mu, logvar, hE, hidden_x = self.encode(x, current_task=kwargs.get('task'))
    ...
    def train_a_batch(self, x, y=None, x_=None, y_=None, scores_=None, tasks_=None, rnt=0.5,
                      active_classes=None, task=1, replay_not_hidden=False, **kwargs):
        ...
        if x_ is not None:
            ...
            if (not type(x_) == list) and (self.mask_dict is None) and (not (self.dg_gates and TaskIL)):
                ...
                recon_batch, y_hat_all, mu, logvar, z = self(x_temp_, gate_input=gate_input, full=True, task=task)
        ...
        # Slowness term
        if self.slowness and task > 1:
            loss_total += 0.00005 * self.habituation_container.get_norm_term()
        ...
        #for name, param in self.fcE.named_parameters():
        ...
        self.habituation_container.reset_norm()
        self.habituation_container.hidden_activations = []
